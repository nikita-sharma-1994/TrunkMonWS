...
router.get('/', function(req, res) {
  ...
  if(typeof req.param('p_TOD_SCHEDULE')  === 'undefined' || typeof req.param('p_TOD_SCHEDULE')  === 'null'){
    res.status(500).json({error : "Missing Parameter : p_TOD_SCHEDULE"}); return;
  }
  var p_TOD_SCHEDULE = "'" + req.param('p_TOD_SCHEDULE') + "'";

  if(typeof req.param('p_Type')  === 'undefined' || typeof req.param('p_Type')  === 'null'){
    res.status(500).json({error : "Missing Parameter : p_Type"}); return;
  }
  var p_Type = "'" + req.param('p_Type') + "'";

  if(typeof req.param('p_Change')  === 'undefined' || typeof req.param('p_Change')  === 'null'){
    res.status(500).json({error : "Missing Parameter : p_Change"}); return;
  }
  var p_Change = req.param('p_Change');
  
  //The SQL query
  var sNote = '[ TGQM=Review; Att={'+ Attempts +'}; Cmp={'+ Completed +'}; Min={'+ Minutes +'}; Ccr={'+ CCR +':0.0%}; Aloc={'+ ALOC +':0.00}; uid={'+ User +'}]'; 

  var params = p_Division + ',' + p_Location + ',' + p_Clli + ',\'' + sNote + '\',' + p_Type + ',' + p_Change + ',' + p_TOD + ',' + p_TOD_SCHEDULE; 
  var rvalue = { rval: {type: oracledb.STRING, dir: oracledb.BIND_OUT} };
  var queryString = "BEGIN trunkmon.intern_package.push_tg_tod_intern("+ params +",:rval); END;";
    ...
    function(err,connection) {
       ...
       connection.execute(queryString, rvalue,
         function(err,result) {
            ...
            res.status(200).json({status : 'Push successful', message : result.outBinds});    
       }); 
  }); 
});
