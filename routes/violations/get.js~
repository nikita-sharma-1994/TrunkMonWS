...
router.get('/', function(req, res) {
  ...
  //The SQL query
  var queryString = "SELECT violation.division_code, division_lookup.division_name, account.account_id, account.last_name, violation.clli, violation.location_id, active_location.description, violation.attempts, violation.completed, (violation.attempts - violation.completed) as failed, violation.minutes, (CASE violation.attempts WHEN 0 then 0 else violation.completed/violation.attempts END) as CCR, (CASE violation.completed WHEN 0 then 0 else violation.minutes/violation.completed END) as ALOC, (CASE trunkmon.threshold.auto_ccr WHEN null then trunkmon.threshold_default.auto_ccr else trunkmon.threshold.auto_ccr END) as AUTO_CCR, (CASE trunkmon.threshold.auto_aloc WHEN null then trunkmon.threshold_default.auto_aloc else trunkmon.threshold.auto_aloc END) as AUTO_ALOC, (CASE trunkmon.threshold.rev_ccr WHEN null then trunkmon.threshold_default.rev_ccr else trunkmon.threshold.rev_ccr END) as REV_CCR, (CASE trunkmon.threshold.rev_aloc WHEN null then trunkmon.threshold_default.rev_aloc else trunkmon.threshold.rev_aloc END) as REV_ALOC, violation.switch_id, v_switch_id.switch_code, v_routing_rule_keep_trunk.account_id as KEEP_TRUNK_ACCOUNT, tod_schedule.schedule_string FROM ((((((((((trunkmon.violation JOIN tel.trunk_group ON trunk_group.trunk_group = violation.trunk_group AND trunk_group.switch_id = violation.switch_id AND trunk_group.direction = 'O') JOIN tel.trunk_assignment ON trunk_assignment.trunk_group_id = trunk_group.trunk_group_id) JOIN telecom.account ON account.account_id = trunk_assignment.account_id) JOIN trunkmon.division_lookup ON violation.division_code = division_lookup.division_code) JOIN trunkmon.active_location ON violation.location_id = active_location.routing_location_id) JOIN (trunkmon.threshold LEFT JOIN trunkmon.threshold_default ON trunkmon.threshold.division_code = trunkmon.threshold_default.division_code) ON violation.division_code = threshold.division_code AND violation.location_id = threshold.routing_location_id) JOIN trunkmon.gen_session ON gen_session.session_id = violation.session_id) JOIN trunkmon.v_switch_id ON v_switch_id.switch_id = violation.switch_id) LEFT JOIN trunkmon.location_schedule ON location_schedule.routing_location_id = violation.location_id) LEFT JOIN trunkmon.tod_schedule ON tod_schedule.schedule_id = location_schedule.schedule_id) LEFT JOIN trunkmon.v_routing_rule_keep_trunk ON trunk_assignment.account_id = v_routing_rule_keep_trunk.account_id AND (division_lookup.super_group_id = v_routing_rule_keep_trunk.super_group_id OR v_routing_rule_keep_trunk.customer_group_id = -1) AND (violation.location_id = v_routing_rule_keep_trunk.routing_location_id OR v_routing_rule_keep_trunk.routing_location_id = -1) AND (active_location.country_id = v_routing_rule_keep_trunk.country_id OR v_routing_rule_keep_trunk.country_id = -1) AND (violation.switch_id = v_routing_rule_keep_trunk.switch_id OR v_routing_rule_keep_trunk.switch_id = -1) AND (violation.switch_id = v_routing_rule_keep_trunk.gateway_id OR v_routing_rule_keep_trunk.gateway_id = -1) AND (trunk_assignment.id = v_routing_rule_keep_trunk.trunk_assignment_id OR v_routing_rule_keep_trunk.trunk_assignment_id = -1) WHERE gen_session.DT=TO_DATE("+ date +",'DD-Mon-YYYY') AND gen_session.HR=" + hour;
